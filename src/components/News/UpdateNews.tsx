...
import { News_Category, newsData } from "@/lib/type";
import Spinner from "../Spinner";
import { UpdateNewsSchema } from "@/lib/shema";
import { useRouter } from "next/navigation";
import { UpdateNews } from "@/app/action/action";
...
const UpdateNewsForm = ({
  ...
  const form = useForm<z.infer<typeof UpdateNewsSchema>>({
    ...
    defaultValues: {
      ...
      category:newsData?.categoryTitle as string,
      file: undefined,
    },
  });

  // 2. Define a submit handler.
  async function onSubmit(values: z.infer<typeof UpdateNewsSchema>) {
    console.log(values);
    try {
      setLoading(true);
      const news = await UpdateNews(newsData?.id as string, values);
      if (news) {
